...
Migrate all `templates` spec files to the atomic test pattern and update `TemplateTestingUtil` to use the updated `MenuItemTestingUtil` API.
...
**Testing util changes (`src/modules/templates/utils/template-testing.util.ts`):**
...
**Note on `seed.service.ts`:** `seed.service.ts` imports `TemplateTestingUtil`. Making `P` optional preserves that callsite. Verify the seed service still compiles and runs after the util change.

**Spec file changes (all 6 files: 2 service, 2 validator, 2 controller — for `template` and `template-menu-item`):**

- Add `const P = \`t${Date.now()}\`` at the top of each `describe` block.
- `beforeAll`: bootstrap the testing module + call the updated template testing util with `P` to create reference data (menu items, templates as needed). Assign returned entities to describe-level variables.
...
- [x] `TemplateTestingUtil` passes `P` to `MenuItemTestingUtil` methods and accepts its own optional `P` parameter; backward compatible with `seed.service.ts`
- [x] Each of the 6 templates spec files declares `const P = \`t${Date.now()}\``
...
- [x] Controller specs contain only the ValidationException wiring test and remove → findOne lifecycle test (template-menu-item controller's create/update endpoints are disabled — `throw new Error('Endpoint not available')` — so only the remove → findOne test applies there, matching the existing precedent in inventory-area-item.controller.spec.ts)
- [x] All `should be defined`, count-comparison `findAll`, and `sortBy` tests are deleted
- [x] `npm run test` passes for all templates spec files
