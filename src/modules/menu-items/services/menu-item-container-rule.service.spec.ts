...
describe('menu item container rule service', () => {
  ...
  beforeAll(async () => {
    ...
    compOptionService = module.get<MenuItemContainerRuleService>(
  ...
  it('should fail to create a container rule', async () => {
    const dto = {} as CreateMenuItemContainerRuleDto;
  ...
  it('should update validItem and size', async () => {
    ...
    if (!toUpdate) {
      throw new Error('comp option is null');
    }
    if (!toUpdate.validItem) {
      throw new Error('comp option valid item is null');
    }
  ...
  it('should update validSizes (add)', async () => {
    ...
    const newItem = await itemService.findOne(toUpdate.validItem.id, [
      'validSizes',
    ]);
    if (!newItem) {
      throw new Error('item b is null');
  ...
  it('should remove a container rule', async () => {
    ...
    await expect(compOptionService.findOne(testId)).rejects.toThrow(
      NotFoundException,
    );
  });
});
