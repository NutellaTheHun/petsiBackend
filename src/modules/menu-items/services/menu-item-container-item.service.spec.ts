...
describe('menu item container item service', () => {
  ...
  it('should update container item', async () => {
    const existing = await containerItemRepo.find({
      take: 1,
      relations: ['parentMenuItem', 'parentItemSize', 'containedMenuItem', 'containedItemSize'],
    });
    if (!existing.length) throw new Error('container item not found');

    const dto: UpdateMenuItemContainerItemDto = { quantity: 10 };

    await dataSource.transaction(async (manager) => {
      await containerItemService.updateEntityForTest(dto, existing[0], manager);
