...
describe('recipe ingredient service', () => {
  ...
  it('should be referenced in recipe query', async () => {
    ...
    expect(
      testRecipe.ingredients?.findIndex((ingred) => ingred.id === testIngredId),
    ).not.toEqual(-1);
  });
  ...
  it('should FAIL create recipe ingredient (inventoryItem and sub recipe)', async () => {
    ...
    await expect(
      recipeService.update(recipeA.id, updateRecipeDto),
  ...
  it('should lose subRecipeIngredient reference', async () => {
    const result = await ingredientService.findOne(testSubRecId, [
  ...
  it('should get all ingredients', async () => {
    const expected =
      await testingUtil.getTestRecipeIngredientEntities(dbTestContext);
  ...
  it('should get a list of ingredients by recipe name', async () => {
    ...
    if (!expected) {
      throw new Error('recipe A not found');
    }
    if (!expected.ingredients) {
      throw new Error('recipe A ingredients are null');
    }
  ...
  it('should remove recipe ingredient', async () => {
    ...
    await expect(ingredientService.findOne(testSubRecId)).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it('should not be referenced in recipe query', async () => {
    ...
    expect(
      testRecipe.ingredients?.findIndex((ingred) => ingred.id === testSubRecId),
    ).toEqual(-1);
  });
  ...
  it('should delete ingredient when deleting recipe', async () => {
    ...
    if (!expected) {
      throw new Error('recipe B not found');
    }
    if (!expected.ingredients) {
      throw new Error('recipe B ingredients are null');
    }

    const ids = (await ingredientService.findByRecipeName(REC_B)).map(
      (ingred) => ingred.id,
    );
  ...
  it('should delete ingredient when deleting inventory item', async () => {
    ...
    const ids = (await ingredientService.findByInventoryItemName(FOOD_A)).map(
      (ingred) => ingred.id,
    );
  ...
  it('should delete ingredient when deleting subRecipe', async () => {
    const ingredient = await ingredientService.findOne(testIngredId, [
      'ingredientRecipe',
    ]);
    if (!ingredient) {
      throw new NotFoundException();
    ...
    await expect(ingredientService.findOne(testIngredId)).rejects.toThrow(
      NotFoundException,
    );
  });
});
