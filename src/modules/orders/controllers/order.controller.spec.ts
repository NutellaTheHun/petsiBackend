...
describe('order controller', () => {
  ...
  beforeAll(async () => {
    ...
    jest
      ...
      .mockImplementation(async (dto: CreateOrderDto) => {
        const oType = types.find((t) => t.id === dto.categoryId);
        const order = {
          id: orderId++,
          category: oType,
    ...
    jest
      ...
      .mockImplementation(async (ids: number[]) => {
        return orders.filter(
          (order) => ids.findIndex((id) => id === order.id) !== -1,
        );
      });
    ...
    jest
      ...
      .mockImplementation(async (id: number, dto: UpdateOrderDto) => {
        ...
        if (dto.categoryId) {
          ...
          orders[existIdx].category = oType;
        }
