...
describe('order menu item controller', () => {
  ...
  beforeAll(async () => {
    ...
    jest
      ...
      .mockImplementation(async (dto: CreateOrderMenuItemDto) => {
        const order = orders.find((o) => o.id === dto.parentOrderId);
        if (!order) {
          throw new Error();
        }
    ...
    jest
      ...
      .mockImplementation(async (id: number, dto: UpdateOrderMenuItemDto) => {
