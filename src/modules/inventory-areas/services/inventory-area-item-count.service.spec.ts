...
import { InventoryItemSizeFactory } from "../../inventory-items/factories/inventory-item-size.factory";
...
describe('Inventory area item count service', () => {
    ...
    let inventoryCountFactory: InventoryAreaCountFactory;
    let itemService: InventoryItemService;
    let itemSizeFactory: InventoryItemSizeFactory;
    ...
    beforeAll(async () => {
        ...
        inventoryCountFactory = module.get<InventoryAreaCountFactory>(InventoryAreaCountFactory);
        const inventoryCounts = await inventoryCountService.initializeTestingDatabase();
        inventoryCountId = inventoryCounts[0].id;
        newInventoryCountId = inventoryCounts[1].id;
        inventoryAreaId = inventoryCounts[0].inventoryArea.id;

        await setupInventoryItemTestingDatabaseLayerTWO(module);

        itemService = module.get<InventoryItemService>(InventoryItemService);
        itemSizeFactory = module.get<InventoryItemSizeFactory>(InventoryItemSizeFactory);
    })
    ...
    it('should create an itemCount', async () => {
        ...
        const dto = itemCountFactory.createDtoInstance({
    ...
    it('should update itemCount', async () => {
        ...
        const updateDto = itemCountFactory.updateDtoInstance({
    ...
    it('should insert many itemCounts for both testing areaCounts', async () => {
        ...
        const items_A: InventoryAreaItemCount[] = [
            itemCountFactory.createEntityInstance({
        ...
        for(const countedItem of items_A){
            const result = await itemCountService.create(
                itemCountFactory.createDtoInstance({
        ...
        const updateResult_A = await inventoryCountService.update(areaCount_A.id, inventoryCountFactory.updateDtoInstance({
            inventoryItemCountIds: countedItemIds_A,
        }));
        ...
        const updateResult_B = await inventoryCountService.update(areaCount_B.id, inventoryCountFactory.updateDtoInstance({
            inventoryItemCountIds: countedItemIds_B,
        }));
