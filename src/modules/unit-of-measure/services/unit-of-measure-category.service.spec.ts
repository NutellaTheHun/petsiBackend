...
describe('UnitOfMeasureCategoryService', () => {
  ...
  it('should set baseUnit to null when unit is deleted', async () => {
    ...
    const category = await categoryService.findOne(testId, [
      'baseConversionUnit',
    ]);
    if (!category) {
  ...
  it('should update categories units list after unit changes category', async () => {
    ...
    if (!unit) {
      throw new Error('couldnt find unit');
    }

    const oldCategoryId = unit?.category?.id;
    if (!oldCategoryId) {
      throw new Error('old category id is null');
    }

    const newCategory = await categoryService.findOneByName(VOLUME);
    if (!newCategory) {
      throw new Error('couldnt find category');
    }
  ...
  it('should update category list after removing a unit of measure', async () => {
    ...
    if (!unitToRemove.category?.id) {
      throw new Error('unit has no category');
    }
    ...
    expect(
      category.units.findIndex((unit) => unit.id == unitToRemove.id),
    ).toEqual(-1);
  });
});
